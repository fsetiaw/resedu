...
public class UpdateQueryClassPoolRulesTable_v1 extends OtakuSQL {
	 ...
	 */
	public Response copyDataClassPoolRulesFromBaseThsms(String token_trans, String tkn_input_value) throws Exception {
		Response res = null;
		boolean valid = false;
		int upd = 0;
		/*
		...
		try {
			...
			if(valid && tkn_input_value!=null) {
				...
				String cmd = "select * from CLASS_POOL_RULES where THSMS=? order by KDPST";
				...
				if(v!=null && v.size()>0) {
					//hapus prev record
					stmt = conn.prepareStatement("delete from CLASS_POOL_RULES where THSMS=?");
					stmt.setString(1, target_thsms);
					stmt.executeUpdate();
					
					//insert new data
					stmt = conn.prepareStatement("insert into CLASS_POOL_RULES (THSMS,KDPST,TKN_VERIFICATOR,URUTAN,KODE_KAMPUS) values (?,?,?,?,?)");
					li = v.listIterator();
					while(li.hasNext()) {
	...
	public Response updateDataClassPoolRules(String token_trans, String tkn_input_value) throws Exception {
		Response res = null;
		boolean valid = false;
		int upd = 0;
		/*
		...
		try {
			...
			if(valid && tkn_input_value!=null) {
				...
				StringTokenizer st = new StringTokenizer(tkn_input_value,"`");
				conn = OtakuSQL.connectToUsg();
				String cmd = "update CLASS_POOL_RULES set TKN_VERIFICATOR=?,URUTAN=? where THSMS=? and KDPST=? and KODE_KAMPUS=?";
				stmt = conn.prepareStatement(cmd);
				String target_thsms = st.nextToken();//thsms buka kelas
